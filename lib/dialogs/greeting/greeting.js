    ...
    constructor(dialogId, userProfileAccessor) {
        ...
        this.verifyBeerDrinkingStep = (step) => __awaiter(this, void 0, void 0, function* () {
            ...
            if (!userProfile.beerDrinker) {
                ...
                return yield step.prompt(BEER_DRINKER, {
         ...
         * @param {PromptValidatorContext} validation context for this validator.
         */
        this.validateBeerDrinker = (validatorContext) => __awaiter(this, void 0, void 0, function* () {
            // Validate that the user entered yes or no
            const value = (validatorContext.recognized.value || '').trim();
            if (value.toLowerCase() === 'yes' || value.toLowerCase() === 'no') {
                return VALIDATION_SUCCEEDED;
            }
            else {
                yield validatorContext.context.sendActivity(`Please click the Yes or No button or type 'Yes' or 'No'`);
                return VALIDATION_FAILED;
            }
        });
        ...
        this.addDialog(new botbuilder_dialogs_1.ChoicePrompt(BEER_DRINKER));
